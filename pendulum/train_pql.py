...
#This paper: https://arxiv.org/abs/2007.08202
def train_PQL_BEAR(state_dim, action_dim, max_action, device, args):
    ...
    replay_buffer.load("./dataset--nt-{}_h-{}_s-{}_a-{}".format(utils.num_trajectories, HORIZON, utils.num_obs_bins, utils.num_act_bins), args.load_buffer_size, bootstrap_dim=4)
    evaluations = []
    training_iters = 0
    print("== Loaded buffer ==")
...
def train_PQL_BCQ(state_dim, action_dim, max_state, max_action, device, args):
    ...
    replay_buffer = utils.Dataset(state_dim, action_dim)
    replay_buffer.load(args.dataset, args.load_buffer_size, bootstrap_dim=4)
    evaluations = []
    filter_scores = []
    print("== Loaded buffer ==")
    training_iters = 0
    while training_iters < args.max_vae_trainstep:
        ...
        print("Start Iteration", training_iters, "in", args.max_vae_trainstep, "iterations")
        vae_loss = policy.train_vae(replay_buffer, iterations=int(args.eval_freq), batch_size=args.batch_size)
    ...
    with open(f'./pqlbcq_policy_{hp_setting}_{log_name}.pkl', 'wb') as outp:
