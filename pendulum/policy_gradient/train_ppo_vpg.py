...
def main(args):
    ...
    def compute_loss_pi(batch):
        ...
        if args.loss_mode == 'vpg':
            ...
            pi_old = torch.exp(logp_old)
            pi_cur = torch.exp(logp)
            loss_pi = -torch.minimum(pi_cur/pi_old*psi, 
            torch.clip(pi_cur/pi_old, 1-args.clip_ratio, 1+args.clip_ratio)*psi).mean()
        else:
        ...
        return loss_pi, pi_info # loss_pi is a tensor

    def compute_loss_v(batch):
        obs, ret = batch['obs'], batch['ret']
        v = ac.v(obs)
        v1 = v.squeeze(-1)
        loss_v = ((v1-ret)**2).mean()
        return loss_v
    ...
    def eval_policy(env='Pendulum-v0', num_test_episodes=10, render=False):
        ...
        for i in range(num_test_episodes):
            ...
            while (t < args.max_ep_len):
                ...
                state = next_state
                t = t+1
                if done or t == args.max_ep_len-1:
                    break
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % TEST_INTERVAL == 0: 
            print('-'*10)
            score = eval_policy(env="Pendulum-v0", render=args.render)
            if score > best_score:
                best_score = score 
                torch.save(ac.state_dict(), "best_model_Pendulum.pt")
                print('######### Saved model to best_model_Pendulum.pt #########')
            print("[TEST Episode {}] [Average Reward {}]".format(epoch, score))
            print('-'*10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Pendulum-v0', help='[CartPole-v0, LunarLander-v2, LunarLanderContinuous-v2, others]')
