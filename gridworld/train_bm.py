...
import gym
import utils
from env import ACTIONS, BOARD_SIZE, GridWorld
import random
...
def init():
    ...
    replay_buffer.load("./qltab_dataset--nt-2000_h-40")
    policy = np.full([BOARD_SIZE]*utils.state_dim+[len(ACTIONS)], 1/len(ACTIONS))
...
def main(args):
    ...
    def f_s_pi(f, s, pi):
        pi_s = pi[int(s[0]), int(s[1]), :]
        pi_s = torch.from_numpy(pi_s).to(torch.float64)
        arr_f = [f(np.array([s[0], s[1], a])) for a in ACTIONS]
    ...
    s0 = np.array([BOARD_SIZE-1, BOARD_SIZE-1]) #init state
    ...
    for t in range(args.T):
        ...
        #updating policy
        for s_id in np.ndindex((BOARD_SIZE,)*utils.state_dim): 
            for a_id in np.ndindex((len(ACTIONS),)*utils.action_dim): 
                s = np.asarray(s_id)
                a = np.asarray(a_id)
                z = np.array([s[0], s[1], a]).astype(float)
